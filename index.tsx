...
import { 
  ...
  Menu, Settings, HelpCircle, Info, MessageSquare, Heart, Share2, Copy, LogOut,
  Moon, Sun, Monitor
} from 'lucide-react';
...
const App = () => {
  ...
  const handleShare = async () => {
    const shareData = {
      title: 'روتینو',
      text: 'با روتینو لحظه رو از دست نده !\nزندگی عالی چیزی نیست جز روتین های عالی . هر عادتی که می خوای روزانه انجام بدی مثل ورزش و کتاب خوندن رو راحت تبدیل به یک روتین کن\n\nدانلود از کافه بازار:',
      url: 'https://cafebazaar.ir/app/com.ace.routino'
    };
    ...
    if (navigator.share) {
      ...
      alert('متن و لینک به کلیپ‌بورد کپی شد!');
    }
    setIsMenuOpen(false);
  };
      ...
      <div className={`fixed inset-y-0 right-0 w-72 bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 z-50 transform transition-transform duration-300 ease-in-out shadow-2xl ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
         <div className="p-6 flex flex-col h-full">
            ...
            <nav className="flex-1 space-y-2">
              <MenuItem icon={<Zap />} label="جدول عادتسازی" active={currentPage === 'home'} onClick={() => { setCurrentPage('home'); setIsMenuOpen(false); }} />
              ...
              <MenuItem icon={<MessageSquare />} label="گزارش مشکلات" active={currentPage === 'feedback'} onClick={() => { setCurrentPage('feedback'); setIsMenuOpen(false); }} />
              <MenuItem icon={<Heart />} label="کمک به برنامه" active={currentPage === 'donate'} onClick={() => { setCurrentPage('donate'); setIsMenuOpen(false); }} />
      ...
      <header className="sticky top-0 z-30 bg-white/80 dark:bg-[#0f172a]/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 px-4 py-3 transition-colors duration-300">
        <div className="max-w-md mx-auto flex justify-between items-center">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-bold text-slate-900 dark:text-white leading-tight mr-1">جدول عادتسازی</h1>
          </div>
            ...
            {habits.length === 0 ? (
              <div className="flex flex-col items-center justify-center text-center py-20 px-6">
                ...
                <p className="text-slate-500 dark:text-slate-500 text-sm mb-8 leading-relaxed max-w-xs mx-auto">
                  اولین قدم را بردارید. عادتی بسازید که آینده شما را می‌سازد.
                </p>
                <button 
                  onClick={() => setIsAddModalOpen(true)}
...
const SplashScreen = () => (
  <div className="fixed inset-0 bg-[#0f172a] z-[100] flex flex-col items-center justify-center animate-out fade-out duration-500 delay-[2500ms]">
     ...
     <h1 className="text-2xl font-bold text-white mt-8 tracking-wider animate-pulse">جدول عادتسازی</h1>
  </div>
);
  ...
  <div className="space-y-4">
    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 transition-colors">
      ...
      <div className="space-y-3">
         <p className="text-xs text-slate-500 dark:text-slate-400 mb-2">تم برنامه</p>
...
const GuidePage = () => (
  <div className="space-y-4 text-justify leading-relaxed">
    <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
       <h3 className="font-bold text-slate-900 dark:text-white mb-3 text-lg">چگونه عادت بسازیم؟</h3>
       <p className="text-slate-600 dark:text-slate-400 text-sm mb-4">
         این برنامه بر اساس اصول کتاب "عادت‌های اتمی" طراحی شده است. ایده اصلی این است که عادت‌های بزرگ را به لقمه‌های کوچک ۴۰ روزه تقسیم کنید.
       </p>
       <ul className="list-disc list-inside space-y-2 text-slate-700 dark:text-slate-300 text-sm marker:text-indigo-500">
         <li><strong>لقمه چله:</strong> هدفی که برای ۴۰ روز آینده دارید.</li>
         <li><strong>لقمه ویژه:</strong> کاری کمتر از هدف اصلی (برای روزهای سخت).</li>
         <li><strong>لقمه اضطراری:</strong> حداقل کاری که باید انجام دهید تا زنجیره قطع نشود.</li>
       </ul>
    </div>
    <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
       <h3 className="font-bold text-slate-900 dark:text-white mb-3 text-lg">قوانین جدول</h3>
       <p className="text-slate-600 dark:text-slate-400 text-sm">
         هر روز فقط می‌توانید وضعیت همان روز را مشخص کنید. اگر روزی را فراموش کنید، قرمز می‌شود. با تکمیل ۴۰ خانه، وارد چله بعدی می‌شوید.
       </p>
    </div>
  </div>
);
...
const AboutPage = () => (
  <div className="space-y-4">
    <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 text-center shadow-sm">
       ...
       <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">درباره ما</h2>
       <p className="text-slate-600 dark:text-slate-400 text-sm leading-7">
         ما تیمی کوچک و علاقه‌مند به توسعه فردی هستیم. هدف ما ایجاد ابزاری ساده و کاربردی برای کمک به فارسی‌زبانان جهت ساخت عادت‌های مثبت و پایدار است.
       </p>
    </div>
  </div>
);
...
const FeedbackPage = () => {
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert("پیام شما با موفقیت ثبت شد (شبیه‌سازی). با تشکر از بازخورد شما!");
  };
    ...
    <div className="space-y-4">
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <Input label="نام و نام خانوادگی" placeholder="" value="" onChange={() => {}} />
          <Input label="شماره تلفن" placeholder="0912..." value="" onChange={() => {}} />
          <Input label="ایمیل (اختیاری)" placeholder="example@mail.com" value="" onChange={() => {}} />
          <div>
            <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1.5 mr-1">متن پیام</label>
            <textarea 
              className="w-full bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 min-h-[120px] transition-colors"
              placeholder="مشکلات یا پیشنهادات خود را بنویسید..."
            ></textarea>
          </div>
          <button className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold shadow-lg shadow-indigo-900/20 mt-2 hover:bg-indigo-700 transition-colors">
            ارسال پیام
          </button>
        </form>
        <p className="text-center text-xs text-slate-400 dark:text-slate-500 mt-4">
          پیام شما مستقیماً برای تیم توسعه ارسال می‌شود.
        </p>
      </div>
    </div>
  );
...
const DonatePage = () => {
  const copyCardNumber = () => {
    navigator.clipboard.writeText("6037997454433294");
    ...
    <div className="space-y-4">
       <div className="bg-gradient-to-br from-indigo-800 to-slate-900 p-6 rounded-2xl border border-indigo-500/30 text-center relative overflow-hidden shadow-lg">
          ...
          <p className="text-slate-300 text-sm leading-7 mb-6">
            این برنامه بدون هیچ چشم‌داشت مالی ساخته شده و کاملاً رایگان است. 
            برای توسعه قابلیت‌های جدید، بهبود سرورها و ادامه فعالیت، نیازمند حمایت‌های شما هستیم.
            کمک‌های مالی شما مستقیماً صرف بهبود تجربه کاربری می‌شود.
          </p>
          ...
          <div className="bg-black/30 p-4 rounded-xl border border-white/10 mb-2 backdrop-blur-sm">
             ...
             <div className="text-xl font-mono text-white tracking-widest font-bold dir-ltr mb-1">6037 9974 5443 3294</div>
    ...
    <div className="bg-white dark:bg-[#1e293b] rounded-2xl shadow-sm dark:shadow-black/20 overflow-hidden relative mb-4 border border-slate-200 dark:border-slate-700/30 transition-colors duration-300">
            ...
            <div className="flex items-center gap-3 pl-1">
                ...
                <div className="flex flex-col items-end justify-center gap-1.5 border-l border-slate-200 dark:border-slate-700/50 pl-3 ml-1">
                   <div className="flex items-center gap-1.5">
                      ...
                      <div className="text-[10px] font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-100 dark:bg-emerald-500/10 px-1.5 py-0.5 rounded">
                         {currentStage.chelehBite}
                      </div>
                   </div>
                   
                   <div className="flex items-center gap-1.5">
                      <span className="text-[9px] text-slate-500 dark:text-slate-400 font-medium">لقمه ویژه</span>
                      <div className="text-[10px] font-bold text-yellow-700 dark:text-yellow-300 bg-yellow-100 dark:bg-yellow-500/10 px-1.5 py-0.5 rounded">
                         {currentStage.specialBite}
                      </div>
                   </div>

                   {currentStage.emergencyBite && (
                     <div className="flex items-center gap-1.5">
                        <span className="text-[9px] text-slate-500 dark:text-slate-400 font-medium">لقمه اضطراری</span>
                        <div className="text-[10px] font-bold text-red-700 dark:text-red-300 bg-red-100 dark:bg-red-500/10 px-1.5 py-0.5 rounded">
                           {currentStage.emergencyBite}
                        </div>
                     </div>
                   )}
        ...
        <div className="border-t border-slate-200 dark:border-slate-700/30 animate-in slide-in-from-top-2 duration-200">
            <div className="flex items-center justify-between px-4 py-3 bg-slate-50 dark:bg-slate-900/30">
                <div className="flex items-center gap-2">
                    <button 
                        ...
                        onClick={(e) => { e.stopPropagation(); setViewingStageIndex(prev => prev - 1); }}
                        className="p-1.5 rounded-lg text-slate-400 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-30 transition-colors"
                    >
                        <ArrowRight size={16} />
                    </button>
                    <span className="text-xs font-medium text-slate-600 dark:text-slate-300 w-16 text-center">
                        چله {viewingStageIndex + 1}
                    </span>
                    <button 
                        disabled={viewingStageIndex === habit.stages.length - 1}
                        onClick={(e) => { e.stopPropagation(); setViewingStageIndex(prev => prev + 1); }}
                        className="p-1.5 rounded-lg text-slate-400 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-30 transition-colors"
                    >
                        <ArrowLeft size={16} />
                ...
                <div className="flex items-center gap-2">
                    <button 
                        ...
                        className="text-slate-500 dark:text-slate-500 hover:text-slate-900 dark:hover:text-slate-300 p-1.5 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700/50 transition-colors"
                    >
            ...
            {isMenuOpen && (
                 <div className="absolute top-28 left-4 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-xl rounded-xl z-20 w-40 py-1 animate-in fade-in zoom-in-95 duration-200">
                    <button onClick={() => { setShowEditModal(true); setIsMenuOpen(false); }} className="w-full text-right px-4 py-2.5 text-slate-600 dark:text-slate-300 text-xs hover:bg-slate-100 dark:hover:bg-slate-700/50 flex items-center gap-2">
                        <Edit2 size={14} /><span>ویرایش</span>
                    </button>
                    <button onClick={() => { setShowResetConfirm(true); setIsMenuOpen(false); }} className="w-full text-right px-4 py-2.5 text-red-500 dark:text-red-400 text-xs hover:bg-red-50 dark:hover:bg-slate-700/50 flex items-center gap-2">
                        <RefreshCw size={14} /><span>اعلام شکست</span>
                    </button>
                    <button onClick={() => { onDelete(habit.id); setIsMenuOpen(false); }} className="w-full text-right px-4 py-2.5 text-slate-500 dark:text-slate-400 text-xs hover:bg-slate-100 dark:hover:bg-slate-700/50 flex items-center gap-2 border-t border-slate-100 dark:border-slate-700/50">
                        <Trash2 size={14} /><span>حذف عادت</span>
                    </button>
                 </div>
            )}
                    ...
                    {viewingStage.days.map((status, index) => {
                        ...
                        return (
                            <button
                                ...
                                className={`aspect-square rounded-md relative flex flex-col items-center justify-center transition-all duration-100 border ${bgClass} ${!editable && !isFilled && !isMissed ? 'opacity-30 cursor-not-allowed' : ''} ${editable ? 'active:scale-90 hover:border-slate-300 dark:hover:border-slate-500/50' : ''}`}
                            >
            ...
            {isStageComplete && isViewingCurrentStage && (
                <div className="p-4 bg-indigo-50 dark:bg-indigo-900/10 border-t border-indigo-100 dark:border-indigo-500/10">
                    <button 
                        ...
                        className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold text-sm shadow-lg shadow-indigo-900/30 flex items-center justify-center gap-2 transition-colors"
                    >
                        <span>شروع چله بعدی</span>
          ...
          <div className="fixed inset-0 z-[60] flex items-end justify-center">
              ...
              <div className="bg-white dark:bg-slate-800 w-full max-w-md rounded-t-[30px] p-6 relative z-10 pb-10 animate-in slide-in-from-bottom duration-300">
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-4">
                      <button 
                          onClick={() => handleSetStatus('special')}
                          className="flex flex-col items-center gap-3 p-4 rounded-2xl bg-yellow-50 dark:bg-yellow-500/10 border border-yellow-200 dark:border-yellow-500/30 text-yellow-600 dark:text-yellow-400 active:bg-yellow-100 dark:active:bg-yellow-500/20 transition-colors"
                      >
                          <div className="w-12 h-12 rounded-full bg-yellow-400 dark:bg-yellow-500 flex items-center justify-center shadow-lg shadow-yellow-900/20 dark:shadow-yellow-900/40">
                              <Circle size={20} className="text-white" strokeWidth={2.5} />
                          </div>
                          <span className="text-xs font-bold">لقمه ویژه</span>
                      </button>
                      <button 
                          onClick={() => handleSetStatus('standard')}
                          className="flex flex-col items-center gap-3 p-4 rounded-2xl bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-emerald-600 dark:text-emerald-400 active:bg-emerald-100 dark:active:bg-emerald-500/20 transition-colors scale-110 origin-bottom"
                      >
                          <div className="w-14 h-14 rounded-full bg-emerald-500 flex items-center justify-center shadow-lg shadow-emerald-900/20 dark:shadow-emerald-900/40">
                              <Check size={28} className="text-white stroke-[3]" />
                          </div>
                          <span className="text-xs font-bold">انجام شد</span>
                      </button>
                      {viewingStage.emergencyBite ? (
                          <button 
                              onClick={() => handleSetStatus('emergency')}
                              className="flex flex-col items-center gap-3 p-4 rounded-2xl bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/30 text-red-600 dark:text-red-400 active:bg-red-100 dark:active:bg-red-500/20 transition-colors"
                          >
                              <div className="w-12 h-12 rounded-full bg-red-500 flex items-center justify-center shadow-lg shadow-red-900/20 dark:shadow-red-900/40">
                                  <X size={24} className="text-white stroke-[3]" />
                              </div>
                              <span className="text-xs font-bold">لقمه اضطراری</span>
                          </button>
                      ) : (
                          <div className="opacity-20 grayscale flex flex-col items-center gap-3 p-4">
                              <div className="w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-600 flex items-center justify-center"><X size={24} className="text-slate-500 dark:text-white" /></div>
      ...
      {showNextStageModal && (
          <AddStageModal 
            ...
            onSave={handleAddNextStage}
            stageNumber={habit.stages.length + 1}
          />
      )}
      ...
      {showEditModal && (
          <EditHabitModal 
            ...
            initialData={{
                title: habit.title,
                initialBite: habit.initialBite,
                finalBite: habit.finalBite,
                chelehBite: viewingStage.chelehBite,
                specialBite: viewingStage.specialBite,
                emergencyBite: viewingStage.emergencyBite
            }}
            isFirstStage={viewingStageIndex === 0}
            stageNumber={viewingStageIndex + 1}
          />
      )}
    </div>
  );
...
// --- Helper Components & Forms (Unchanged mostly) ---
